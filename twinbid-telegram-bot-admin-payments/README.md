...
`409 Conflict`. Бот бесшумно закрывает callback, удаляет кнопки у исходного
сообщения и отправляет в чат обычное сообщение
`⚠️ Модерация уже отменена пользователем`.
