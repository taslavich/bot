...
func (b *BotClient) postCampaignMultipart(ctx context.Context, req CampaignModerationRequest) error {
	...
	for i, cr := range req.Creatives {
		...
		partHeader.Set("Content-Disposition", fmt.Sprintf(`form-data; name="creative_image_%d"; filename="%s"`, i, escapeQuotes(filename)))
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, b.BaseURL+"/internal/campaigns/moderation", &body)
	if err != nil {
		return err
	}
...
func (b *BotClient) postJSON(ctx context.Context, path string, payload any) error {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, b.BaseURL+path, bytes.NewBuffer(body))
	if err != nil {
		return err
	}
